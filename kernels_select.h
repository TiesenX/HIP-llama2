...
void gpu_rmsnorm(float* o, float* x, float* weight, int size, int batch_size, hipStream_t *stream) {
  // for (int idx = 0; idx < batch_size; idx++)
  rmsnorm_kernel <<<batch_size, num_threads_lrg, 0, *stream>>> (o, x, weight, size);
}
...
void gpu_matmul(float *xout, float *x, float *w, int N, int D, int batch_size, hipStream_t *stream)
{
    ...
    dim3 gridSize(D);
    matmulGPU<<<gridSize, threadBlock, 0, *stream>>>(xout, x, w, N, D);
    CHECK_HIP(hipGetLastError());
}
...
void gpu_mat_vec(float *xout, float *x, float *w, int n, int d, hipStream_t *stream)
{
    // W (d,n) @ x (n,) -> xout (d,)
    // by far the most amount of time is spent inside this little function
    ...
    dim3 gridSize(d);
    matvecGPU<<<gridSize, threadBlock, 0, *stream>>>(xout, x, w, n, d);
    CHECK_HIP(hipGetLastError());
}
...
void gpu_swiglu(float *shb, float *shb2, int hidden_dim, int batch_size, hipStream_t *stream) {
    ...
    swiglu_kernel<<<num_blocks, threads_per_block, 0, *stream>>>(shb, shb2, hidden_dim, batch_size);
}
...
void gpu_accum(float *a, const float *b, int size, int batch_size, hipStream_t *stream) {
    int threads_per_block = 256;
    int num_blocks = min((size * batch_size + threads_per_block - 1) / threads_per_block, 1024);
    accum_kernel<<<num_blocks, threads_per_block, 0, *stream>>>(a, b, size, batch_size);
}
